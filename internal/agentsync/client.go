...
import (
	...
	storagecatalog "github.com/share-disk/share-disk/internal/storage/catalog"
...
func (c *Client) Provision(ctx context.Context, callerAuthorization, callerUserID string) error {
	...
	c.mu.Lock()
	defer c.mu.Unlock()
	body := identity.RegisterDeviceRequest{Name: c.deviceName, Platform: "linux", PeerID: c.peerID}
	var auth identity.AuthResponse
...
func (c *Client) Enroll(ctx context.Context, account, password string) error {
	...
	request := identity.EnrollDeviceRequest{Account: account, Password: password, Name: c.deviceName, Platform: "linux", PeerID: c.peerID}
	var auth identity.AuthResponse
...
func (c *Client) persistAuth(ctx context.Context, auth identity.AuthResponse) error {
...
func (c *Client) executeOneDeviceCommand(ctx context.Context) {
	...
	var command catalog.DeviceCommand
...
func (c *Client) applyDeviceCommand(ctx context.Context, command catalog.DeviceCommand) error {
...
func (c *Client) executeOneTransfer(ctx context.Context) {
	...
	var assignment transfer.Assignment
	...
	report := transfer.CompleteRequest{ExpectedVersion: version, LocalFileID: localFile.ID, Endpoint: c.endpoint, Size: info.Size, SHA256: info.Hash.String()}
...
func (c *Client) recordTransfer(a transfer.Assignment) {
...
func (c *Client) downloadReplica(ctx context.Context, assignment transfer.Assignment, dst *os.File) error {
	info, err := dst.Stat()
	if err != nil {
		return err
	}
...
func (c *Client) refresh(ctx context.Context) error {
	...
	var auth identity.AuthResponse
	status, _, err := c.request(ctx, http.MethodPost, "/v1/auth/refresh", "", identity.RefreshRequest{RefreshToken: refreshToken}, &auth)
